...
## Correctif - types des clés de substitution dans `create_tables.sql`

`create_tables.sql` déclarait jusqu'ici toutes les colonnes `*_key` en `BIGINT`. Ce
n'est plus le type réel produit par le pipeline : depuis le passage à des clés
textuelles (`CLI_000001`, `DATE_00001`, `ACC_0000001`...) dans
`01_etl/etl_pipeline/dimensions.py`, ce sont des chaînes de caractères. Cette
divergence était invisible tant que le chargement se fait via `load_warehouse.py`
(SQLAlchemy infère le type automatiquement depuis le DataFrame), mais aurait fait
échouer un chargement utilisant `create_tables.sql` comme DDL explicite. **Corrigé** :
toutes les colonnes `*_key` (primaires et étrangères) sont maintenant en `VARCHAR(20)`.

## `date_key` - nullité vérifiée, pas un bug

`fact_account_event.date_key` peut être `NULL` - c'est structurel, pas une clé
étrangère orpheline (voir `01_etl/README.md`, section "`date_key` - vérification
explicite de la nullité"). Le commentaire du DDL indiquait auparavant "~22,5%", une
valeur qui provenait en fait de `notbooks ETL/02_ETL.ipynb` (le notebook narratif),
pas du pipeline de production. **Chiffre vérifié et corrigé** contre
`etl_pipeline/` (qui applique une correction supplémentaire sur `ACCT_OPENING_DATE`
que le notebook n'applique pas encore, cf. `01_etl/README.md`) : **134 056 lignes,
soit 30,1%** de `fact_account_event`, strictement égal au nombre d'`ACCT_OPENING_DATE`
manquantes en amont - un contrôle explicite dans `fact.py` lève désormais une erreur
si cette égalité ne tient plus (signe d'un vrai échec de jointure).