...
d'extraction, valeurs aberrantes). Six mod\`eles de classification (test\'es
avec et sans SMOTE, soit dix configurations) ont \'et\'e compar\'es au grain
client (319~129 clients, taux de churn 36\%)~; le mod\`ele retenu, XGBoost avec
SMOTE, est s\'electionn\'e sur la base du \textbf{recall} (84,92\%, le meilleur
parmi les mod\`eles \`a pr\'ecision exploitable), avec un F1-score de 88,24\% et
une AUC-ROC de 94,64\%, apr\`es qu'une fuite de donn\'ees
(\texttt{acct\_balance}, manquante pour 100\% des clients churn\'es) a \'et\'e
d\'etect\'ee et exclue en cours de construction. Une analyse compl\'ementaire non
supervis\'ee, au m\^eme grain client, identifie quatre segments par K-means et
...
and corrected. Six classification models (tested with and without SMOTE, ten
configurations in total) were compared at client grain (319,129 clients,
36\% churn rate); the selected model, XGBoost with SMOTE, is chosen on the
basis of \textbf{recall} (84.92\%, the best among models with usable
precision), reaching an F1-score of 88.24\% and a ROC-AUC of 94.64\%, after a
data-leakage issue (\texttt{acct\_balance}, missing for 100\% of churned
clients) was detected and excluded during development. A complementary
unsupervised analysis, at the same client grain, identifies four K-means
segments and 3,192 anomalous clients via Isolation Forest.
...
Le mod\`ele supervis\'e est entra\^in\'e au grain \textbf{client}
(\texttt{client\_key}), avec la d\'efinition du churn agr\'eg\'ee par
\texttt{min} (churn = 1 si et seulement si tous les comptes du client sont
ferm\'es --- voir section~\ref{ch:donnees}). La table de faits est agr\'eg\'ee par
client avant tout entra\^inement, ramenant le jeu de donn\'ees \`a
\textbf{319~129 lignes}, pour un taux de churn de \textbf{36\%} --- un
d\'es\'equilibre de classes mod\'er\'e plut\^ot qu'extr\^eme.
...
\begin{itemize}
\item \texttt{client\_key} --- identifiant, sans valeur pr\'edictive.
\item \texttt{acct\_balance} --- \textbf{fuite d'information} (voir
      section~\ref{sec:leakage}).
\item \texttt{nb\_comptes} --- redondante avec \texttt{nb\_produits}
      (99,1\% de correspondance entre les deux colonnes).
\item \texttt{NATIONALITY}, \texttt{RESIDENCE} --- quasi constantes
      (majoritairement \texttt{"TN"}).
\end{itemize}

Le jeu de variables retenu compte 6 variables num\'eriques
(\texttt{salary}, \texttt{amount\_total}, \texttt{fixedrate\_mean},
\texttt{nb\_produits}, \texttt{age}, \texttt{salary\_missing}) et 8 variables
cat\'egorielles (\texttt{MARITAL\_STATUS}, \texttt{NATURE\_CLIENT},
\texttt{PARTYCLASS}, \texttt{LOB}, \texttt{INDUSTRY}, \texttt{SCORE\_KYC},
\texttt{COMPLETED\_FILE}, \texttt{BRANCH}).

\section{Fuite de donn\'ees d\'etect\'ee et corrig\'ee}
\label{sec:leakage}

La v\'erification syst\'ematique du taux de churn selon qu'une variable est
renseign\'ee ou non (m\'ethode appliqu\'ee \`a chaque colonne pr\'esentant des
valeurs manquantes) a r\'ev\'el\'e un cas de fuite \'evident~: parmi les clients
pour lesquels \texttt{acct\_balance} est manquant, \textbf{100\%} ont churn\'e.
Une variable parfaitement corr\'el\'ee \`a l'absence de donn\'ee pour la classe
positive n'est pas un signal m\'etier exploitable en production (elle
encoderait, de fait, le label lui-m\^eme) --- \texttt{acct\_balance} a donc
\'et\'e exclue des variables explicatives.

\`A l'inverse, la m\^eme v\'erification sur \texttt{salary} n'a pas r\'ev\'el\'e de
fuite~: le taux de churn est proche, que le salaire soit renseign\'e
(35,57\%) ou manquant (37,69\%). \texttt{salary\_missing} a donc \'et\'e
conserv\'e comme indicateur binaire l\'egitime, et \texttt{salary} lui-m\^eme
imput\'e par la m\'ediane plut\^ot que laiss\'e manquant.

Une v\'erification similaire sur \texttt{COMPLETED\_FILE} (dossier client
complet ou non) confirme un signal l\'egitime et fort~: 49,6\% de churn parmi
les dossiers incomplets contre 22,5\% parmi les dossiers complets --- une
corr\'elation plausible d'un point de vue m\'etier (un dossier incomplet
signale un engagement client plus faible), retenue comme variable
explicative.

\begin{quote}
Cette d\'emarche --- v\'erifier syst\'ematiquement le taux de churn conditionnel
\`a la pr\'esence/absence de chaque variable avant de l'utiliser --- est la
m\^eme discipline que celle qui avait permis de d\'etecter une fuite li\'ee \`a
l'anciennet\'e du compte lors d'une it\'eration ant\'erieure du mod\`ele au grain
compte~: elle est d\'esormais syst\'ematique dans le pipeline de mod\'elisation
de l'\'equipe.
\end{quote}

\section{Pr\'etraitement}

Les variables cat\'egorielles \`a faible cardinalit\'e (\texttt{MARITAL\_STATUS},
\texttt{NATURE\_CLIENT}, \texttt{PARTYCLASS}, \texttt{SCORE\_KYC},
\texttt{COMPLETED\_FILE}, \texttt{LOB}) sont encod\'ees en \emph{one-hot}~;
\texttt{BRANCH} (141 valeurs) et \texttt{INDUSTRY} (637 valeurs) sont
encod\'ees par fr\'equence, ajust\'ee uniquement sur l'ensemble d'entra\^inement
pour \'eviter toute fuite. Le jeu de donn\'ees est s\'epar\'e 80/20 de mani\`ere
stratifi\'ee, et SMOTE~\citep{chawla2002smote} est appliqu\'e uniquement sur
l'ensemble d'entra\^inement pour r\'e\'equilibrer les classes --- chaque mod\`ele
est test\'e \`a la fois avec et sans SMOTE, soit dix configurations au total.

\section{Mod\`eles compar\'es et r\'esultats}

Six mod\`eles ont \'et\'e entra\^in\'es~: r\'egression logistique, K plus proches
voisins (KNN), arbre de d\'ecision, for\^et al\'eatoire~\citep{breiman2001random},
machine \`a vecteurs de support (SVM) et
XGBoost~\citep{chen2016xgboost}, chacun avec et sans SMOTE.
...
\toprule
\textbf{Mod\`ele} & \textbf{Precision} & \textbf{Recall} & \textbf{F1-score} & \textbf{ROC-AUC} & \textbf{Temps (s)} \\
\midrule
\textbf{XGBoost (avec SMOTE)} & 91,82\% & \textbf{84,92\%} & \textbf{88,24\%} & 94,64\% & 2,84 \\
XGBoost (sans SMOTE) & 92,23\% & 84,48\% & 88,19\% & \textbf{94,74\%} & 1,97 \\
Random Forest (avec SMOTE) & 91,67\% & 84,83\% & 88,12\% & 94,37\% & 24,08 \\
Random Forest (sans SMOTE) & 92,35\% & 84,30\% & 88,14\% & 94,40\% & 16,15 \\
Arbre de d\'ecision (sans SMOTE) & 92,17\% & 84,14\% & 87,97\% & 94,16\% & 1,22 \\
KNN (avec SMOTE) & 55,20\% & 68,83\% & 61,27\% & 74,30\% & 1,23 \\
SVM (avec SMOTE, sous-\'ech.) & 53,99\% & 67,77\% & 60,10\% & 71,04\% & 437,22 \\
KNN (sans SMOTE) & 62,98\% & 53,11\% & 57,63\% & 75,01\% & 0,06 \\
Arbre de d\'ecision (avec SMOTE) & 37,41\% & 98,19\% & 54,17\% & 49,77\% & 1,76 \\
SVM (sans SMOTE, sous-\'ech.) & 60,00\% & 47,89\% & 53,27\% & 70,57\% & 115,38 \\
\bottomrule
\end{tabular}
\caption{Comparaison des mod\`eles de classification (grain client, 10 configurations avec/sans SMOTE)}
\label{tab:ml-results}
\end{table}

L'analyse d'importance des variables d'XGBoost identifie \texttt{amount\_total}
comme largement dominante (52,7\% du poids de d\'ecision), loin devant
\texttt{nb\_produits} (7,7\%), \texttt{COMPLETED\_FILE = YES} (6,1\%),
\texttt{LOB} = 30 (4,7\%) et \texttt{fixedrate\_mean} (3,5\%).

\section{Crit\`ere de s\'election~: pourquoi le recall prime sur ce probl\`eme}

Le F1-score et le ROC-AUC restent de bons indicateurs globaux, mais le crit\`ere
d\'ecisif retenu par l'\'equipe pour arbitrer entre les mod\`eles les plus
performants a \'et\'e le \textbf{recall} (rappel) de la classe churn, pour une
raison m\'etier pr\'ecise~: dans un contexte de r\'etention bancaire, le co\^ut
d'un \textbf{faux n\'egatif} (un client qui va r\'eellement churner mais que le
mod\`ele classe comme fid\`ele) est structurellement plus \'elev\'e que celui d'un
\textbf{faux positif} (un client contact\'e inutilement par un conseiller). Ne
pas d\'etecter un churn r\'eel signifie perdre le client sans avoir tent\'e la
moindre action de r\'etention~; contacter \`a tort un client fid\`ele co\^ute au
pire un appel superflu.

Sur ce crit\`ere, \textbf{XGBoost avec SMOTE obtient le meilleur recall parmi
les mod\`eles dont les autres m\'etriques restent exploitables} (84,92\%),
devant Random Forest avec SMOTE (84,83\%) et XGBoost sans SMOTE (84,48\%).
L'\'ecart avec ces deux challengers est faible en valeur absolue, mais XGBoost
avec SMOTE l'emporte \'egalement sur le F1-score (88,24\%, le meilleur du
comparatif) et reste tr\`es rapide \`a entra\^iner (2,84 secondes) --- un
compromis recall/pr\'ecision/temps d'ex\'ecution qui n'est domin\'e sur aucun
axe par un autre mod\`ele non d\'eg\'en\'er\'e.

\begin{quote}
\textbf{Point de vigilance m\'ethodologique}~: le recall le plus \'elev\'e du
comparatif appartient en r\'ealit\'e \`a l'arbre de d\'ecision avec SMOTE
(98,19\%) --- mais ce chiffre est trompeur. Sa pr\'ecision s'effondre \`a
37,41\% et son ROC-AUC tombe \`a 49,77\%, soit \`a peine mieux que le hasard~:
ce mod\`ele maximise le recall en pr\'edisant le churn de mani\`ere quasi
syst\'ematique, ce qui n'a aucune valeur op\'erationnelle (une \'equipe de
r\'etention ne peut pas contacter la quasi-totalit\'e de la base client). Ce
r\'esultat illustre pourquoi le recall doit toujours \^etre lu conjointement
avec la pr\'ecision et le ROC-AUC, jamais isol\'ement.
\end{quote}

\textbf{Mod\`ele retenu~: XGBoost avec SMOTE.} Il maximise le recall parmi les
mod\`eles dont la pr\'ecision reste exploitable, tout en offrant le meilleur
F1-score et un temps d'entra\^inement compatible avec un r\'eentra\^inement
r\'egulier en production.

\section{Limitations assum\'ees}

KNN et SVM sont entra\^in\'es/\'evalu\'es sur un sous-\'echantillon (co\^ut de
pr\'ediction et noyau RBF trop \'elev\'es sur 319~129 clients dans le temps
imparti), ce qui limite la comparabilit\'e stricte de leurs r\'esultats avec les
quatre autres mod\`eles, entra\^in\'es sur le jeu complet. Cette limitation est
document\'ee explicitement dans le notebook.
...
Ce grain est d\'esormais le m\^eme que celui utilis\'e au chapitre~\ref{ch:ml}
pour la mod\'elisation supervis\'ee~: les deux analyses partagent la m\^eme
population et la m\^eme d\'efinition du churn, mais r\'epondent \`a des questions
diff\'erentes et compl\'ementaires. La mod\'elisation supervis\'ee r\'epond \`a
\emph{qui} va probablement churner (une pr\'ediction individuelle, exploit\'ee
par l'application de scoring du chapitre~\ref{ch:webapp}). L'analyse non
supervis\'ee ci-dessous r\'epond \`a \emph{quels profils} coexistent dans la
base et \emph{quels clients} s'\'ecartent de tout profil standard --- une
lecture structurelle que la seule pr\'ediction binaire ne fournit pas.
...
Le mod\`ele retenu (XGBoost avec SMOTE, chapitre~\ref{ch:ml}) est restitu\'e via
une application Streamlit~\citep{streamlitdocs} \`a quatre pages~: un \'ecran
d'accueil pr\'esentant un r\'esum\'e du jeu de donn\'ees, un formulaire de
scoring individuel (saisie des caract\'eristiques d'un client et
...
Le mod\`ele charg\'e est li\'e aux features d\'efinies au grain \textbf{client}
(chapitre~\ref{ch:ml})~: le formulaire de scoring doit donc saisir des
caract\'eristiques agr\'eg\'ees au niveau client (ex.\ \texttt{nb\_produits},
\texttt{amount\_total} cumul\'e), pas des caract\'eristiques d'un compte isol\'e.
Si une \'evolution future du notebook de mod\'elisation change \`a nouveau de
grain ou de jeu de variables, \texttt{model.pkl} et
...
Le mod\`ele XGBoost avec SMOTE retenu (recall = 84,92\%, F1 = 88,24\%,
AUC-ROC = 94,64\%) offre une base solide pour un d\'eploiement op\'erationnel,
sous r\'eserve des am\'eliorations identifi\'ees (validation crois\'ee,
explicabilit\'e par instance). Les tableaux de bord Power BI et l'application
Streamlit compl\`etent le dispositif en rendant les r\'esultats accessibles \`a
un public m\'etier non technique, condition n\'ecessaire \`a l'adoption effective
d'un tel outil par les \'equipes de r\'etention client.
